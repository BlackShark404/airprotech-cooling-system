        ...
        columns.push({
          ...
          render: (data, type, row) => {
            ...
            if (this.options.viewRowCallback) {
              actionsHtml += `<button class="btn btn-sm btn-info view-btn me-1" data-id="${row.id}"><i class="bi bi-eye"></i></button>`;
            }
            
            if (this.options.editRowCallback) {
              actionsHtml += `<button class="btn btn-sm btn-warning edit-btn me-1" data-id="${row.id}"><i class="bi bi-pencil"></i></button>`;
            }
            
            if (this.options.deleteRowCallback) {
              actionsHtml += `<button class="btn btn-sm btn-danger delete-btn" data-id="${row.id}"><i class="bi bi-trash"></i></button>`;
            }
    ...
    _attachEventListeners() {
      ...
      if (this.options.deleteRowCallback) {
        table.on('click', '.delete-btn', (e) => {
          ...
          // Call delete callback (which should handle confirmation)
