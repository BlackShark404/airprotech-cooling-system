...
CREATE TABLE SERVICE_BOOKING (
    ...
    SB_REQUESTED_DATE   DATE NOT NULL,
    SB_REQUESTED_TIME   TIME NOT NULL,
...
-- PRODUCT_ORDER Table: Records customer product orders
CREATE TABLE PRODUCT_ORDER (
    PO_ID               SERIAL PRIMARY KEY,
    PO_CUSTOMER_ID      INT NOT NULL,
    PO_VARIANT_ID       INT NOT NULL,
    PO_QUANTITY         INT NOT NULL CHECK (PO_QUANTITY > 0),
    PO_UNIT_PRICE       DECIMAL(10, 2) NOT NULL,
    PO_TOTAL_AMOUNT     DECIMAL(10, 2) GENERATED ALWAYS AS (PO_QUANTITY * PO_UNIT_PRICE) STORED,
    PO_STATUS           VARCHAR(20) DEFAULT 'pending',
    PO_ORDER_DATE       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PO_PAID_DATE        TIMESTAMP,
    PO_CREATED_AT       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PO_UPDATED_AT       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PO_DELETED_AT       TIMESTAMP,
    CONSTRAINT FK_ORDER_CUSTOMER FOREIGN KEY (PO_CUSTOMER_ID)
        REFERENCES CUSTOMER(CU_ACCOUNT_ID) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT FK_ORDER_VARIANT FOREIGN KEY (PO_VARIANT_ID)
        REFERENCES PRODUCT_VARIANT(VAR_ID) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT CK_ORDER_STATUS CHECK (PO_STATUS IN ('pending', 'confirmed', 'completed', 'cancelled'))
);

-- PRODUCT_ASSIGNMENT Table: Assigns technicians to product orders
CREATE TABLE PRODUCT_ASSIGNMENT (
    ...
    CONSTRAINT FK_ASSIGNMENT_ORDER FOREIGN KEY (PA_ORDER_ID)
        REFERENCES PRODUCT_ORDER(PO_ID) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- --------------------------------------
-- 5. Triggers for User and Role Management
-- --------------------------------------
